...
int CreateHTTPserver()
{
	...
	if (pid == 0) // child process
	{
                ...
                else if(!strcmp(strHTTP_requestPath, "/compute"))
		{
			...
			std::vector<double> calculatedByFunctionValues;
                        Calculation calculation;
                        int x = 0.9; 
			for (int i=0; i<2950; i++) {
				calculatedByFunctionValues.push_back(calculation.FuncA(x, i));
			}

			for (int i=0; i<500; i++)
			{
				sort(begin(calculatedByFunctionValues), end(calculatedByFunctionValues));
				reverse(begin(calculatedByFunctionValues), end(calculatedByFunctionValues));
			}
